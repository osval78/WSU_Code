...
unique(BLUEs$Year)
unique(BLUEs$Loc)
BLUEs_2019=BLUEs[BLUEs$Year==2019,]
BLUEs_2019=droplevels(BLUEs_2019)
dim(BLUEs_2019)
length(unique(BLUEs_2019$Hybrid_Name))

BLUEs_2020=BLUEs[BLUEs$Year==2020,]
BLUEs_2020=droplevels(BLUEs_2020)
dim(BLUEs_2020)
length(unique(BLUEs_2020$Hybrid_Name))

BLUEs_2021=BLUEs[BLUEs$Year==2021,]
BLUEs_2021=droplevels(BLUEs_2021)
dim(BLUEs_2021)
length(unique(BLUEs_2021$Hybrid_Name))
boxplot( GY~Env+Year,
        data=BLUEs,
...
for (e in seq(folds)){
 ...
 #   e=1
   ...
   fold <- folds[[e]]
    ...
    ######input of lines
    ETA_Line=list(Env=list(K=K.E, model="RKHS"),Line=list(K=K.L, model="RKHS") )
    y2[pos_test_e]=NA
    model_Line <-BGLR(
      ETA=ETA_Line,
      y=y2,
  ...
  ######input of Images
  ETA_Image=list(Env=list(K=K.E, model="RKHS"),Image=list(X=X_Image, model="BRR") )
  y2[pos_test_e]=NA
  model_Image <-BGLR(
    ETA=ETA_Image,
    y=y2,
  ...
  ##Combinig Latent variables
  ETA_Line_Image=list(Env=list(K=K.E, model="RKHS"),Line=list(K=K.L, model="RKHS"),Image=list(X=X_Image, model="BRR") )
  y2[pos_test_e]=NA
  model_Line_Image <-BGLR(
    ETA=ETA_Line_Image,
    y=y2,
    nIter =10000,
    burnIn= 2500,
  ...
  Pred_Joint=model_Line_Image$yHat[pos_test_e] 
  Pred_Joint_trn=model_Line_Image$yHat[-pos_test_e] 

  #plot(y_tst,Pred_Joint)
  cbind(y_tst,Pred_Joint)
  cor(y_tst,Pred_Joint)
  
  ###Saving predictions of Images
  Predictions_Joint_e=data.frame(Line=Pheno$Hybrid_Name[pos_test_e], Fold=e, Env=Pheno$Env[pos_test_e], Observed=y_tst,Predicted=Pred_Joint)
  Predictions_Joint=rbind(Predictions_Joint,Predictions_Joint_e)
  ...
  Pred_Env_Summary_Joint=gs_summaries(Predictions_Joint)$env
  All_summary_Joint=rbind(All_summary_Joint,data.frame(cbind(Modality="Joint",EC=w, Trait_t=Trait_t,Pred_Env_Summary_Joint)))
...
All_summary=rbind(Summary_All_EC_Line,Summary_All_EC_Image,  Summary_All_EC_Joint)
write.csv(All_summary,file="Carter_BGLR_Model1_Predictor_E+G+EC_7FCV_tst.csv")
