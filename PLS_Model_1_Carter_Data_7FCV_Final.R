...
for(w in 1:3){
 # w=1
  ...
  K.LE=K.L*K.E
  X_LE=t(chol(K.LE))
  X1=cbind(X_Env,X_Line)
  X2=cbind(X_Env,X_Image)
  X3=cbind(X_Env,X_Line,X_Image)
  #head(Pheno)
...
#    e=1
  ...
  T_joint_trn=X3[-pos_test_e, ]
  T_joint_tst=X3[pos_test_e, ]
...
write.csv(All_summary,file="Carter_PLS_Model1_Predictor_E+G+EC_7FCV_tst.csv")
