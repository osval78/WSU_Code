...
#    e=1
    ...
    ######input of lines
    ETA_Line=list(Line=list(K=K.L, model="RKHS") )
    y2[pos_test_e]=NA
    model_Line <-BGLR(
      ETA=ETA_Line,
      y=y2,
  ...
  ######input of Images
  ETA_Image=list(Image=list(X=X_Image, model="BRR") )
  y2[pos_test_e]=NA
  model_Image <-BGLR(
    ETA=ETA_Image,
    y=y2,
  ...
  ETA_Line_Image=list(Line=list(K=K.L, model="RKHS"),Image=list(X=X_Image, model="BRR") )
  y2[pos_test_e]=NA
...
write.csv(All_summary,file="Carter_BGLR_Predictor_G+EC_PLS_Multimoda_Final_V9_tst.csv")
