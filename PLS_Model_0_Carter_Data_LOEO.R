...
#    e=1
    tst_e=Envs[e]
    # tst_e
    pos_test_e=which(Pheno$Env %in% tst_e)
    ####Response variable
    y_trn <- y[-pos_test_e]
    y_tst<-y[pos_test_e]
    y2=y
    ...
    ######input of lines
    X_trn_Line <-X_Line[-pos_test_e, ]
    X_tst_Line <-X_Line[ pos_test_e, ]
  ...
  ######input of Images
  X_trn_Image <-X_Image[-pos_test_e, ]
  X_tst_Image <-X_Image[ pos_test_e, ]
  ...
  T_joint_trn=data.matrix(cbind(X_trn_Line,X_trn_Image))
  T_joint_tst=data.matrix(cbind(X_tst_Line,X_tst_Image))
  PLS_res_Joint=PLS_Individual(X_trn=T_joint_trn,y_trn=y_trn,X_tst=T_joint_tst)
  Pred_Joint=PLS_res_Joint$Pred
  Pred_Joint_trn=PLS_res_Joint$Pred_trn
...
write.csv(All_summary,file="Carter_PLS_Predictor_G+EC_PLS_Multimoda_Final_V9_tst.csv")
